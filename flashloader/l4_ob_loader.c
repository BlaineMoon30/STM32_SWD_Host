/*
 * STM32L4 mini flashloader for option-byte (RDP) programming.
 *
 * This code is uploaded by the STM32G0 SWD host to the target STM32L4's
 * SRAM and executed there. Because the writes to KEYR / OPTKEYR / OPTR
 * happen from the L4's own CPU (native bus master), the flash controller
 * accepts them even under RDP Level 1 protection — which is the trick
 * ST-Link / STM32CubeProgrammer uses.
 *
 * The STM32L4 flash IP is the newer controller shared with G0/G4/WB/U0:
 ...
 * encoding and OBL_LAUNCH are byte-for-byte identical to the STM32G4 (and
 * G0), so this loader is functionally the same as g4_ob_loader.c. L4 is a
 * dual-bank part, but the RDP byte is in OPTR[7:0] regardless of bank, so a
 * read-modify-write of just that byte is all this loader needs.
 *
