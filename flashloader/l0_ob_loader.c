/*
 * STM32L0 mini flashloader for option-byte (RDP) programming.
 *
 * This code is uploaded by the STM32G0 SWD host to the target STM32L0's
 * SRAM and executed there. Because the writes to PEKEYR / OPTKEYR / OB
 * happen from the L0's own CPU (native bus master), the flash controller
...
#define FLASH_PECR     (*(volatile uint32_t *)0x40022004UL)
#define FLASH_PEKEYR   (*(volatile uint32_t *)0x4002200CUL)
#define FLASH_OPTKEYR  (*(volatile uint32_t *)0x40022014UL)
#define FLASH_SR       (*(volatile uint32_t *)0x40022018UL)
...
void l0_ob_loader_entry(void)
{
   ...
   * STM32L0 OB programming requires explicit erase before program:
   *   set PECR.ERASE -> write 0 to OB address -> wait BSY -> clear PECR.ERASE
   * Skipping this step makes the subsequent OB write fail silently and the
   * hardware complement-verify rejects the resulting word at OBL_LAUNCH.
   */
