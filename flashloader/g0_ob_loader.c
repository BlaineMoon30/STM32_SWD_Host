/*
 * STM32G0 mini flashloader for option-byte (RDP) programming.
 *
 * This code is uploaded by the STM32G0 SWD host to the target STM32G0's
 * SRAM and executed there. Because the writes to KEYR / OPTKEYR / OPTR
 * happen from the G0's own CPU (native bus master), the flash controller
 * accepts them even under RDP Level 1 protection — which is the trick
 * ST-Link / STM32CubeProgrammer uses.
 *
 * Unlike the STM32L0 flash IP (PECR/PEKEYR, BSY = bit 0), the STM32G0 uses
 * the newer flash controller shared with G4/L4/WB:
 *   - FLASH_CR.LOCK / OPTLOCK gate register / option-byte writes
 *   - the RDP level lives in FLASH_OPTR[7:0]
 *   - FLASH_SR.BSY is bit 16 (NOT bit 0)
