/*
 * STM32G4 mini flashloader for option-byte (RDP) programming.
 *
 * This code is uploaded by the STM32G0 SWD host to the target STM32G4's
 * SRAM and executed there. Because the writes to KEYR / OPTKEYR / OPTR
 * happen from the G4's own CPU (native bus master), the flash controller
 * accepts them even under RDP Level 1 protection — which is the trick
 * ST-Link / STM32CubeProgrammer uses.
 *
 * The STM32G4 flash IP is identical to the STM32G0 one (shared with G4/L4/WB):
 * same register base (0x40022000) and offsets, same keys, RDP in OPTR[7:0],
 * FLASH_SR.BSY = bit 16, commit with CR.OPTSTRT, reload with CR.OBL_LAUNCH.
 * This loader is therefore the same code as g0_ob_loader.c — kept as a
 * separate family file for clarity and future divergence (e.g. dual-bank G4).
 *
 * The loader does a read-modify-write of only the RDP byte in OPTR so that
 * every other user option bit (BOR, watchdog, nBOOT, dual-bank, etc.) is
 * preserved.
 *
